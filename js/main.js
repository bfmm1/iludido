...
function draw() { ctx.clearRect(0, 0, canvas.width, canvas.height); switch(estadoDoJogo) { case 'carregando': ctx.fillStyle = '#fff'; ctx.font = '40px "Courier New", monospace'; ctx.textAlign = 'center'; ctx.fillText(`Carregando...`, canvas.width / 2, canvas.height / 2); break; case 'menuPrincipal': drawMainMenu(); break; case 'menuPoderes': drawPowersMenu(); break; case 'rodando': case 'entreOndas': plataformas.forEach(p => p.draw()); projeteis.forEach(p => p.draw()); projeteisInimigos.forEach(p => p.draw()); player.draw(); inimigos.forEach(i => i.draw()); efeitosVisuais.forEach(e => e.draw()); drawGameUI(); break; case 'levelUp': plataformas.forEach(p => p.draw()); projeteis.forEach(p => p.draw()); projeteisInimigos.forEach(p => p.draw()); player.draw(); inimigos.forEach(i => i.draw()); efeitosVisuais.forEach(e => e.draw()); drawGameUI(); drawLevelUpScreen(); break; case 'gameOver': drawGameOverScreen(); break; } ctx.textAlign = 'start'; }
